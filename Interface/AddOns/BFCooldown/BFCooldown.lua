...
local CD = BFCooldown
CD.style = {
    "Interface\\Cooldown\\star4", "Interface\\Cooldown\\ping4",
...
function CD.Util:GetSpellID(spell)
    ...
    if BFCooldownDB.spellCache[spell] then
        if BFCooldownDB.spellCache[spell][2] == "spell" then
            if type(BFCooldownDB.spellCache[spell][1]) == "number" and
                (GetSpellInfo(BFCooldownDB.spellCache[spell][1], "spell")) ==
                spell then
                return unpack(BFCooldownDB.spellCache[spell]);
            end
        elseif BFCooldownDB.spellCache[spell][2] == "pet" then
            if type(BFCooldownDB.spellCache[spell][1]) == "number" and
                (GetSpellInfo(BFCooldownDB.spellCache[spell][1], "pet")) ==
                spell then
                return unpack(BFCooldownDB.spellCache[spell]);
            end
        else
...
function CD.Util:FormatCenterText(text)
    ...
    return format(L.CDOver,
                  format("|cff%2x%2x%2x%s|r", BFCooldownDB.center.r * 255,
                         BFCooldownDB.center.g * 255,
                         BFCooldownDB.center.b * 255, text));
end
...
function CD.Util:GetTimeString(BFCooldown_ac970a92b1bae84f591d2098084a233d)
    ...
    local BFCooldown_377682ab4d11f1b730b680e0c65e174c =
        math.floor(BFCooldown_ac970a92b1bae84f591d2098084a233d / 60);
    local BFCooldown_c7b591b2c752c8cb9c847770ee2d8f9d =
        BFCooldown_ac970a92b1bae84f591d2098084a233d -
            BFCooldown_377682ab4d11f1b730b680e0c65e174c * 60;
...
local function spellEqual(s1, s2)
    if (s1.name and s2.name and s1.name == s2.name) or
        (s1.spell and s2.spell and s1.spell == s2.spell) then return true end
    return false
end
OO.Spell = {}
...
function OO.Spell:IsValid()
    if self.id and self.duration and self.duration > 2 then return true end
    return false
end
...
function OO.Spell:BindUI(ui) self.ui = ui end
function OO.Spell:UnbindUI() end
function OO.Spell:OnExpiring() centerList:AddSpell(self) end
...
function OO.Spell:GetAcc() return self.count end
...
function OO.SpellList:HasSpell(spell)
    for _i, _spell in pairs(self.data) do
        if _spell == spell then return _spell end
    end
    return false
end
function OO.SpellList:AccSpell(spell) spell.count = spell.count + 1 end
...
function OO.SpellList:RemoveSpell(spell)
    for _i, _spell in pairs(self.data) do
        if _spell == spell then tremove(self.data, _i) end
    end
end
function OO.SpellList:BindUI(ui) end
function OO.SpellList:Count() return #self.data end
function OO.SpellList:Get(_i) return self.data[_i] end
...
function CD:UpdateCenterStyle(frame)
    if frame then
        ...
        frame:SetPoint(BFCooldownDB.center.position.p, UIParent,
        ...
        frame.text:SetFont(BFCooldownDB.center.font, BFCooldownDB.center.size,
                           "OUTLINE");
...
function CD:UpdateCenter()
    if BFCooldownDB.center.config and centerList:Count() > 0 and
        (not self.center:IsVisible()) then
...
function CD:UpdateShine(shine, elapsed)
    ...
    local scale = (BFCooldownDB[shine.type].scale - 1) * (1 - shine.completed) +
                      1;
    if scale <= 1 then
        actives[shine] = nil;
        shine:Hide();
...
function CD:UpdateAllShines(elapsed)
    if next(actives) then
        for shine in pairs(actives) do self:UpdateShine(shine, elapsed); end
    else
        self:Hide();
    end
end
local function IsSpecialButton(button, type)
    ...
    if BFCooldownDB.ACTION.special then return false end
    if not button:GetName() then return false end
    return string.find(button:GetName(), "Totem") or
               string.find(button:GetName(), "Rune")
end
function CD:SetCooldown(cooldown, start, duration)
    ...
    if cooldown.button then
        cooldown.type = cooldown.type or
                            self.Util:GetButtonType(cooldown.button);
        if not IsSpecialButton(cooldown.button, cooldown.type) then
            if cooldown.type then
                if start > 0 and duration > 2 and
                    BFCooldownDB[cooldown.type].config then
...
function CD:UpdateTimer(timer)
    timer.cooldown:SetAlpha(
        BFCooldownDB[timer.cooldown.type].hidecooldown and 0 or 1);
    ...
    elseif _time > 0.4 then
        local str, scale, r, g, b, nextUpdate =
            self.Util:GetFormattedTime(_time,
                                       BFCooldownDB[timer.cooldown.type].long);
        ...
        if size <= 0 then
            ...
            timer.text:SetFont(BFCooldownDB[timer.cooldown.type].font, size,
                               "OUTLINE");
            timer.text:SetText(str);
            timer.text:SetTextColor(r, g, b);
            timer.text:SetAlpha(BFCooldownDB[timer.cooldown.type].alpha);
            timer.text:SetPoint("CENTER", timer, BFCooldownDB[timer.cooldown
                                    .type].point or "CENTER", 0, 0);
            timer.text:Show();
...
function CD:CreateTimer(cooldown)
    ...
    text:SetPoint("CENTER", timer,
                  BFCooldownDB[cooldown.type].point or "CENTER", 0, 0);
    timer.text = text;
...
function CD.Timer.TF:Create(guid, list)
    ...
    t.ui.lock = BLibrary("BFLock", ui, ui, BFCooldownDB, L.CoolDownBar,
                         L.LockToolTip)
    t.ui.lock:Show()
    return t
end
...
function CD.Timer.TF:Update()
    if BFCooldownDB.bar and #(self.timers) > 0 then
        self.ui:Show()
        for _i, _timer in pairs(self.timers) do
            if _timer and _timer.Update then _timer:Update() end
        end
        self:Arrange()
    else
        self.ui:Hide()
...
function CD.Timer.ST:Create(spell)
    ...
    local ui = _G["SpellTimerSpellFrame" .. spell:GetName()] or
                   CreateFrame("Frame",
                               "SpellTimerSpellFrame" .. spell:GetName(),
                               UIParent, "BFCDSpellFrameTemplate")
    t.ui = ui
    t:Bind(spell)
    return t
end
...
function CD.Timer.ST:Update()
    if not self.spell:IsExpired() then
        self.text:SetText(
            "|cff00ffbb" .. CD.Util:GetTimeString(self.spell.left) .. "-" ..
                self.spell:GetName() .. "|r")
        self.bar:SetStatusBarColor(1.0, 0.7, 0.0)
        self.bar:SetMinMaxValues(self.spell.start,
                                 self.spell.start + self.spell.duration)
        self.bar:SetValue(self.spell.start + self.spell.duration -
                              self.spell.left)
    else
...
function CD:LoadConfig()
    BFCooldownDB =
        (BFCooldownDB and BFCooldownDB.isChangeFont and BFCooldownDB) or
            self:GetDefault();
...
function CD:HookCooldown()
    local methods = getmetatable(CreateFrame("Cooldown", nil, nil,
                                             "CooldownFrameTemplate")).__index;
    ...
    abEventWatcher:SetScript('OnEvent', function(self, event)
        for cooldown in pairs(active) do cooldown_Update(cooldown) end
    end)
...
function CD:ToggleMiddle(tog) BFCooldownDB.center.config = tog end
function CD:ToggleBar(tog) BFCooldownDB.bar = tog end
function CD:SwitchActionStyle(tog) BFCooldownDB.ACTION.style = tog end
function CD:ToggleSec(tog, type) BFCooldownDB[type].config = tog end
function CD:ToggleSpecial(tog) BFCooldownDB.ACTION.special = tog end
local function CD_Update(self, elapsed) CD:Update(elapsed) end
